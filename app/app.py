...
def get_vids(query: str, top: int) -> List[int]:
...
async def process_query(query: str, top_n: int, meta: List[Dict], path: str, client: yadisk.Client):
    query = t.translate(query)
    vid_idxs = get_vids(query, top_n)
    for idx in vid_idxs:
...
async def main():
    ...
    if token and path and meta_file:
        ...
        if query:
            await process_query(query, top_n, meta, path, client)
